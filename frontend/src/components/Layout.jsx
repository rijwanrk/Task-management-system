...
const Layout = ({ user, onLogout }) => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axios.get("https://task-management-system-backend-cceq.onrender.com/api/tasks/gp", {
  ...
  const StatCard = ({ title, value, icon }) => (
    <div className="p-2 sm:p-3 rounded-xl bg-white shadow-sm border border-blue-100 hover:shadow-md transition-all duration-300 hover:border-blue-200 group">
      <div className="flex items-center gap-2">
        <div className="p-1.5 rounded-lg bg-gradient-to-br from-fuchsia-500/10 to-blue-500/10 group-hover:from-fuchsia-500/20 group-hover:to-blue-500/20">
          {icon}
        </div>
